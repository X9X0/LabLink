...
class EquipmentStatus(BaseModel):
    ...
    capabilities: dict = Field(
        default_factory=dict, description="Equipment-specific capabilities"
    )

    # Diagnostic fields (v0.12.0)
    temperature_celsius: Optional[float] = Field(
        None, description="Current equipment temperature in Celsius"
    )
    operating_hours: Optional[float] = Field(
        None, description="Cumulative operating hours"
    )
    error_code: Optional[int] = Field(None, description="Last equipment error code")
    error_message: Optional[str] = Field(
        None, description="Last equipment error message"
    )
    last_calibration_date: Optional[datetime] = Field(
        None, description="Date of last calibration"
    )
    calibration_due_date: Optional[datetime] = Field(
        None, description="Date calibration is due"
    )
    calibration_status: Optional[str] = Field(
        None, description="Calibration status (current, due, overdue)"
    )
    self_test_status: Optional[str] = Field(
        None, description="Built-in self-test status"
    )
    self_test_last_run: Optional[datetime] = Field(
