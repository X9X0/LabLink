...
class DataStreamConfig(BaseModel):
    ...
    stream_type: str = Field(..., description="Type of data to stream (waveform, measurements, etc)")
    interval_ms: int = Field(100, description="Update interval in milliseconds", ge=10, le=10000)
    enabled: bool = Field(True, description="Whether streaming is enabled")
    buffer_size: int = Field(1000, description="Number of samples to buffer before writing")
