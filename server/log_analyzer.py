...
class LogAnalyzer:
    ...
    def analyze_performance(entries: List[LogEntry]) -> Dict[str, Any]:
        ...
        perf_entries = [
            e for e in entries if "duration_ms" in e.extra or "memory_mb" in e.extra
        ]
        ...
        # Duration statistics
        durations = [
            e.extra["duration_ms"] for e in perf_entries if "duration_ms" in e.extra
        ]
        duration_stats = None
        if durations:
            duration_stats = {
        ...
        # Memory statistics
        memory_values = [
            e.extra["memory_mb"] for e in perf_entries if "memory_mb" in e.extra
        ]
        memory_stats = None
        if memory_values:
            memory_stats = {
    ...
    def detect_anomalies(
        entries: List[LogEntry], sensitivity: float = 2.0
    ) -> List[Dict[str, Any]]:
        ...
        if error_entries:
            ...
            for entry in error_entries:
                interval_key = entry.timestamp.replace(
                    minute=(entry.timestamp.minute // 5) * 5, second=0, microsecond=0
                )
            ...
            if len(intervals) > 1:
                ...
                for interval, count in intervals.items():
                    if stdev_errors > 0 and count > mean_errors + (
                        sensitivity * stdev_errors
                    ):
                        anomalies.append(
                            {
                                "type": "error_spike",
                                ...
                                "severity": (
                                    "high"
                                    if count > mean_errors + (3 * stdev_errors)
                                    else "medium"
                                ),
                            }
                        )
        ...
        if len(perf_entries) > 10:
            ...
            for entry in perf_entries:
                ...
                if stdev_duration > 0 and duration > mean_duration + (
                    sensitivity * stdev_duration
                ):
                    anomalies.append(
                        {
                            "type": "slow_operation",
...
class LogFormatter:
    ...
    def format_entry_text(entry: LogEntry, show_extra: bool = True) -> str:
        ...
        location = (
            f"{entry.module}:{entry.function}:{entry.line}" if entry.module else ""
        )
...
def cmd_query(args):
    ...
    if args.level:
        ...
        print(
            f"Filtered by level {args.level}: {len(entries)} entries", file=sys.stderr
        )
    ...
    if args.output_format == "json":
        ...
        output = "\n\n".join(
            formatter.format_entry_text(e, show_extra=args.show_extra) for e in entries
        )
...
Examples:
  ...
  python log_analyzer.py anomaly --sensitivity high
    ...
    query_parser.add_argument(
        "--keyword", nargs="+", help="Filter by keywords in message"
    )
    ...
    query_parser.add_argument(
        "--case-sensitive", action="store_true", help="Case-sensitive keyword search"
    )
    query_parser.add_argument(
        "--show-extra", action="store_true", default=True, help="Show extra fields"
    )
    query_parser.add_argument(
        "--output-format",
    ...
    report_parser.add_argument(
        "--type",
        choices=["summary", "errors", "performance"],
        default="summary",
        help="Report type",
    )
    report_parser.add_argument(
    ...
    report_parser.add_argument(
        "--file-pattern", default="*.log", help="Log file pattern"
    )
