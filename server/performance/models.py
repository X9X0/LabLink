...
class PerformanceBaseline(BaseModel):
    ...
    baseline_id: str = Field(
        default_factory=lambda: f"baseline_{uuid.uuid4().hex[:12]}"
    )
    equipment_id: Optional[str] = Field(
        None, description="Equipment ID (None for system-wide)"
    )
    ...
    latency_critical_threshold_ms: float = Field(
        ..., description="Latency critical threshold"
    )
    throughput_warning_threshold: float = Field(
        ..., description="Throughput warning threshold"
    )
    error_rate_warning_threshold: float = Field(
        default=5.0, description="Error rate warning %"
    )
    error_rate_critical_threshold: float = Field(
        default=10.0, description="Error rate critical %"
    )
    ...
    measurement_period_hours: float = Field(
        ..., description="Measurement period in hours"
    )
...
class PerformanceMetric(BaseModel):
    ...
    deviation_percent: Optional[float] = Field(
        None, description="% deviation from baseline"
    )
    within_threshold: Optional[bool] = Field(
        None, description="Within acceptable threshold"
    )
...
class PerformanceTrend(BaseModel):
    ...
    predicted_value_1h: Optional[float] = Field(
        None, description="Predicted value in 1 hour"
    )
    ...
    time_to_threshold: Optional[float] = Field(
        None, description="Hours until threshold breach"
    )
...
class PerformanceReport(BaseModel):
    ...
    report_id: str = Field(
        default_factory=lambda: f"perf_report_{uuid.uuid4().hex[:12]}"
    )

    # Scope
    equipment_ids: List[str] = Field(
        default_factory=list, description="Equipment included"
    )
    components: List[str] = Field(
        default_factory=list, description="Components analyzed"
    )
    ...
    # Overall status
    overall_status: PerformanceStatus = Field(
    ...
    total_measurements: int = Field(
        default=0, description="Total measurements analyzed"
    )
    ...
    # Comparison to baseline
    latency_vs_baseline_percent: Optional[float] = Field(
        None, description="Latency vs baseline %"
    )
    throughput_vs_baseline_percent: Optional[float] = Field(
        None, description="Throughput vs baseline %"
    )

    # Trends
    latency_trend: Optional[TrendDirection] = Field(None, description="Latency trend")
    throughput_trend: Optional[TrendDirection] = Field(
        None, description="Throughput trend"
    )
    error_rate_trend: Optional[TrendDirection] = Field(
        None, description="Error rate trend"
    )

    # Issues and recommendations
    active_alerts: int = Field(default=0, description="Number of active alerts")
    degraded_components: List[str] = Field(
        default_factory=list, description="Components with degraded performance"
    )
    critical_components: List[str] = Field(
        default_factory=list, description="Components with critical performance"
    )
    recommendations: List[str] = Field(
    ...
    component_performance: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Per-component performance details"
    )
    trend_analyses: List[PerformanceTrend] = Field(
...
class PerformanceThresholds(BaseModel):
    ...
    latency_warning_ms: float = Field(
        default=100.0, description="Latency warning threshold"
    )
    latency_critical_ms: float = Field(
        default=500.0, description="Latency critical threshold"
    )

    # Throughput thresholds (commands/sec)
    throughput_warning: float = Field(
        default=10.0, description="Throughput warning threshold"
    )
    throughput_critical: float = Field(
        default=5.0, description="Throughput critical threshold"
    )
    ...
    degradation_warning: float = Field(
        default=20.0, description="Performance degradation warning %"
    )
    degradation_critical: float = Field(
        default=50.0, description="Performance degradation critical %"
    )

    # Measurement window
    measurement_window_minutes: int = Field(
        default=60, description="Measurement window in minutes"
    )
    min_samples_required: int = Field(
        default=10, description="Minimum samples for analysis"
    )
