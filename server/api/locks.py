...
from server.equipment.locks import (LockManager, LockMode, LockViolation,
                                    lock_manager)
from server.equipment.sessions import (SessionInfo, SessionManager,
                                       session_manager)
...
class AcquireLockRequest(BaseModel):
    ...
    timeout_seconds: int = Field(
        default=300, ge=0, description="Lock timeout in seconds (0 = no timeout)"
    )
...
class CreateSessionRequest(BaseModel):
    ...
    timeout_seconds: int = Field(
        default=600, ge=0, description="Session timeout in seconds"
    )
...
@router.get(
    ...
    summary="Get equipment lock status",
)
...
@router.get(
    ...
    summary="Get equipment lock queue",
)
async def get_queue_status(equipment_id: str):
    ...
    try:
        ...
        return QueueStatusResponse(
            equipment_id=equipment_id, queue=[entry.dict() for entry in queue]
        )
...
async def get_all_locks():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get all locks: {str(e)}"
        )
...
async def get_lock_events(
    ...
    try:
        ...
        return {"equipment_id": equipment_id, "events": events, "count": len(events)}

    except Exception as e:
        logger.error(f"Error getting lock events: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get lock events: {str(e)}"
        )
...
@router.post(
    "/sessions/create", response_model=SessionResponse, summary="Create new session"
)
...
@router.get(
    "/sessions/{session_id}", response_model=SessionResponse, summary="Get session info"
)
...
async def get_session_locks(session_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get session locks: {str(e)}"
        )
...
async def get_session_events(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get session events: {str(e)}"
        )
