...
class FilterConfig(BaseModel):
    ...
    filter_method: FilterMethod = Field(
        FilterMethod.BUTTERWORTH, description="Filter design method"
    )
    cutoff_freq: Optional[float] = Field(None, description="Cutoff frequency (Hz)")
    cutoff_low: Optional[float] = Field(
        None, description="Low cutoff for bandpass/stop (Hz)"
    )
    cutoff_high: Optional[float] = Field(
        None, description="High cutoff for bandpass/stop (Hz)"
    )
    order: int = Field(4, ge=1, le=10, description="Filter order")
    ripple_db: Optional[float] = Field(
        None, description="Passband ripple (dB) for Chebyshev"
    )
    attenuation_db: Optional[float] = Field(
        None, description="Stopband attenuation (dB)"
    )
...
class ResampleConfig(BaseModel):
    ...
    method: ResampleMethod = Field(
        ResampleMethod.LINEAR, description="Interpolation method"
    )
...
class FitConfig(BaseModel):
    ...
    initial_guess: Optional[List[float]] = Field(
        None, description="Initial parameter guess"
    )
    bounds: Optional[List[List[float]]] = Field(
        None, description="Parameter bounds [[lower], [upper]]"
    )
    weights: Optional[List[float]] = Field(None, description="Data point weights")
    custom_function: Optional[str] = Field(
        None, description="Custom function (Python code)"
    )
...
class SPCChartConfig(BaseModel):
    ...
    sigma_limits: float = Field(
        3.0, gt=0, description="Number of standard deviations for control limits"
    )
...
class CapabilityResult(BaseModel):
    ...
    # Yield estimates
    expected_within_spec: Optional[float] = Field(
        None, description="% within spec limits"
    )
    expected_defects_ppm: Optional[float] = Field(
        None, description="Defects per million"
    )
...
class BatchJobConfig(BaseModel):
    ...
    operation_config: Dict[str, Any] = Field(
        ..., description="Operation-specific config"
    )
