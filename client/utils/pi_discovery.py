"""Client-side Raspberry Pi network discovery."""
...
class PiDiscovery:
    """Network discovery for Raspberry Pi devices (client-side)."""
    ...
    async def _ping_host(self, ip: str, timeout: float) -> tuple[bool, Optional[float]]:
        ...
        """
        try:
            # Use system ping command (works on Linux/Mac/Windows with different args)
            import platform
        ...
        try:
            ...
            for line in output.split("\n"):
                if ip in line:
                    parts = line.split()
                    for part in parts:
                        # Check for MAC address format (XX:XX:XX:XX:XX:XX or XX-XX-XX-XX-XX-XX)
                        if (":" in part or "-" in part) and (len(part) == 17):
